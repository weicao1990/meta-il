...
class ResampleEnv:
    ...
    def _fit_model(self, X, y, index_pos, index_neg):
        ...
        model.fit(train_X, train_y, eval_set=[
                  (self.valid_X, self.valid_y)], eval_metric=custom_metric, early_stopping_rounds=50, verbose=10)
        return model
...
class PPO:
    ...
    def train(self):
        final_state = torch.Tensor(self.memory.states[-1]).unsqueeze(dim=0)
        final_reward, _ = self.policy.evaluate(final_state, None)
        discount_r = 0.0

        td_target_list = []
        for r in self.memory.rewards[::-1]:
            discount_r = gamma * discount_r + r
            td_target_list.append([discount_r])
        td_target_list.reverse()

        td_target = torch.FloatTensor(td_target_list).to(device)

        s, a, r, logp = self.make_batch()

        for i in range(nb_epoches):
            s_value, a_logprob = self.policy.evaluate(s, a)

            advantage = td_target - s_value.detach()
